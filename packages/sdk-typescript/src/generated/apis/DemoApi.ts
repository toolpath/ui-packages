...
export class DemoApi extends runtime.BaseAPI {
  ...
  /**
   * Issues a short-lived API key for anonymous, no-signup access to a limited set of endpoints. Each session is isolated to its own throwaway organization, so it sees only the data it uploaded. Send the same `installId` again to renew: the previous key is retired and a new one is issued on the same organization. The `installId` is therefore a bearer secret, held to the same care as the key it mints.
